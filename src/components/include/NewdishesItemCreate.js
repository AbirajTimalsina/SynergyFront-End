...
export default function ModalItemCreate() {
	...
	const handleFileChange = (e) => {
		...
		axios
			...
			.then((response) => {
				setitems({
					newdishesname: items.itemname,
					newdishesprice: items.itemprice,
					newdishespicture: response.data.filename,
				});
			})
	...
	const create = (e) => {
		...
		console.log(items);
		axios
			.post('http://localhost:3000/newdishesfood/', items)
			.then((response) => {
				toggle();
	...
	return (
		<div>
			...
			style={{ width: 150, marginLeft: 825 }} onClick={toggle}>
				Create Item
			</Button>
